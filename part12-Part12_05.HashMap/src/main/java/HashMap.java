...
public class HashMap <K, V> {
    ...
    public V get(K key) {
        ...
        for (int i = 0; i < valuesAtIndex.size(); i++) {
            if (valuesAtIndex.get(0).getKey().equals(key)) {
                return valuesAtIndex.get(0).getValue();
            }
        }
        return null;
    }
    ...
    private void grow() {
        ArrayList<Pair<K,V>>[] newValues = new ArrayList[this.values.length + this.values.length/2];
